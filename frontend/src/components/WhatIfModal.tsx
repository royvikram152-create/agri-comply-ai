...
export const WhatIfModal: React.FC<WhatIfModalProps> = ({ isOpen, onClose, shipmentId }) => {
  const [destination, setDestination] = useState('European Union');
  const [deadlineDays, setDeadlineDays] = useState(7);
  const [residueValue, setResidueValue] = useState(0.31);
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 bg-slate-800/90 border-b border-slate-700/60 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-bold text-slate-100 text-lg">Phase K: What-If Analysis Simulation</h3>
              <p className="text-xs text-slate-400">Non-destructive parameter trade simulation</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Residue (mg/kg)</label>
              <input
                type="number"
                step="0.01"
                value={residueValue}
          ...
          >
            ...
            <span>{loading ? 'Simulating Pipeline...' : 'Run What-If Simulation'}</span>
          </button>

          {result && (
            <div className="mt-4 p-4 bg-slate-950 border border-indigo-500/30 rounded-xl space-y-3">
              <div className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Simulation Outcome vs Current</div>
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div className="p-3 bg-slate-900 border border-slate-800 rounded-lg">
                  <div className="text-slate-400">Current Real Shipment</div>
                  <div className="text-sm font-bold text-rose-400 mt-1">HOLD (0.82 mg/kg)</div>
                  <div className="text-slate-400 text-[10px] mt-0.5">1 Critical Violation</div>
                </div>
                <div className="p-3 bg-emerald-950/40 border border-emerald-500/30 rounded-lg">
                  ...
                  <div className="text-sm font-bold text-emerald-300 mt-1">{result.simulated_outcome.status}</div>
                  <div className="text-emerald-400 text-[10px] mt-0.5">Score: {result.simulated_outcome.compliance_score} / 100</div>
                </div>
              </div>
            </div>
          )}
