...
import { Package, ShieldAlert, CheckCircle2, AlertTriangle, ArrowRight, Activity, Calendar } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ shipments, onSelectShipment, onNewShipmentClick }) => {
  ...
  const holdCount = shipments.filter(s => s.status === 'HOLD').length;
  const readyCount = shipments.filter(s => s.status === 'READY_FOR_APPROVAL' || s.status === 'APPROVED').length;
  const reviewCount = shipments.filter(s => s.status === 'REVIEW_REQUIRED').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 p-6 rounded-2xl border border-slate-800 shadow-xl">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">Real-time agentic inspection, MRL safety validation, and deterministic compliance firewall.</p>
        </div>
        <button
          onClick={onNewShipmentClick}
          className="px-4 py-2 bg-brand-600 hover:bg-brand-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-brand-900/40 flex items-center space-x-2 transition"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          value={holdCount}
          subtitle="Critical gap detected"
      ...
      <div className="bg-slate-950/60 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">Click shipment to view readiness & evidence chain</span>
        </div>
            ...
            <tbody className="divide-y divide-slate-800/60 text-slate-200">
              {shipments.map((s) => (
                <tr key={s.id} className="hover:bg-slate-900/50 transition">
                  ...
                  <td className="py-4 px-4 text-slate-300 font-medium flex items-center mt-3">
                  ...
                  <td className="py-4 px-4 text-right">
                    <button
                      ...
                      className="px-3 py-1.5 bg-brand-600/20 hover:bg-brand-600 text-brand-400 hover:text-white border border-brand-500/30 rounded-lg text-xs font-semibold inline-flex items-center space-x-1 transition"
                    >
